...
| Credencial | Informação apresentada para comprovar a identidade de um usuário | O enunciado menciona usuário e senha. Para a validação, a [`DEC-0005`](../arquitetura/decisoes/0005-keycloak-no-ambiente-de-validacao.md) atribui contas e autenticação por senha ao Keycloak; a aplicação não recebe a senha, valida o token OIDC e obtém `(issuer, subject)` como identidade autenticada. Conforme [`REQ-CHG-0002`](refinamentos/REQ-CHG-0002.md), a direção de autogestão está validada, mas cadastro, recuperação e seu primeiro recorte permanecem `A confirmar`. | Declarado; realização técnica decidida; direção futura `Validada na descoberta`; primeiro recorte `A confirmar` |
...
| Estado do trabalho | Fase vigente do ciclo de vida de um trabalho de vídeo | No modelo arquitetural vigente, [`CMP-20 — Ciclo do Trabalho`](../arquitetura/componentes-coesos.md#cmp-20) é a única autoridade e o único escritor do estado; [`CMP-21 — Processamento de Mídia`](../arquitetura/componentes-coesos.md#cmp-21) relata fatos correlacionados sem alterá-lo, conforme [`DEC-0004`](../arquitetura/decisoes/0004-componentes-coesos-do-nucleo.md) | Inferido |
