...
| `CMP-02` | Trabalhos de Vídeo | Possuir o ciclo de vida de uma solicitação de processamento | Validar e aceitar submissão; gerar identidade única; associar proprietário; validar transições; autorizar ciclos e reprocessamento; listar trabalhos; autorizar resultado; aplicar retenção | Extrair frames, executar `ffmpeg`, implementar banco/fila/storage, entregar notificação | Autoridade sobre trabalho, proprietário, estado, datas, ciclos e referências opacas dos artefatos; consome `IdentidadeAutenticada`; solicita `ProcessarTrabalho`; aceita `ResultadoDoProcessamento`; solicita `NotificarFalha` |
| `CMP-03` | Processamento de Mídia | Transformar um vídeo aceito em um resultado reproduzível | Consumir tentativa autorizada; isolar execução; extrair frames; gerar ZIP; preservar o resultado; classificar e relatar sucesso ou falha; tolerar reentrega | Autenticar usuário, aceitar submissão, decidir propriedade ou ciclo, listar estado, escolher a transição persistente do trabalho | Autoridade sobre execução transitória e política de extração; consome `ProcessarTrabalho`; fornece `ResultadoDoProcessamento` correlacionado; depende de referências opacas de artefato e processador externo |
...
| US-04 | Trabalhos de Vídeo | Processamento de Mídia; mecanismo durável de entrega | Preservar trabalho e controlar ciclos, reprocessamento e transições idempotentes |
...
| Trabalhos de Vídeo | Processamento de Mídia | Temporal; dependência estática a confirmar | A medir | IDs do trabalho, ciclo e tentativa, referência da origem e parâmetros de extração; não executor nem quantidade de consumidores | `ProcessarTrabalho` | Trabalho aceito e origem preservada precedem cada tentativa autorizada; chamada síncrona obrigatória propagaria falhas e uniria os quanta |
| Processamento de Mídia | Trabalhos de Vídeo | Temporal; dependência estática a confirmar | A medir | Correlação, referência do resultado ou falha classificada como transitória/permanente; não máquina de estados, proprietário ou política de ciclos | `ResultadoDoProcessamento` | Resultado ou falha preservados precedem a transição; evitar ciclo de implementação por porta ou fato de integração idempotente |
