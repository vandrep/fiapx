...
O modelo histórico [`CTX-CMP-002`](../historico/componentes/ctx-cmp-002-componentes-modulares.md) demonstrou que uma unidade de implantação não pode ser inferida por componente. O sucessor ativo [`CTX-CMP-003`](../componentes-coesos.md), aceito por [`DEC-0004`](0004-componentes-coesos-do-nucleo.md), consolida oito componentes lógicos. A [arquitetura `CTX-ARCH-001`](../comparacao-e-arquitetura-recomendada.md) analisou a candidata física; esta decisão é quem agrupa os componentes em três quanta. As três abstrações permanecem distintas:
